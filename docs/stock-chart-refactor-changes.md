...
> 명백한 결함은 사용자 판정 후에만 별도 커밋으로 수정한다. 이번 사이클의 의미 변경은 승인된 결함 **a·b 두 건뿐**이다.
...
- `tier-d-mappers`(`mapMinuteToStockChartDto`, `mapToStockChartDto` 추출) — 로케일 의존 축자 보존이 핵심이라 성격이 달라 별도 사이클로 연기.
