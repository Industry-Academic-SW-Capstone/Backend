...
- **클래스·메서드 위에 설명 블록을 두지 않습니다.** 필요하면 한 줄로 끝냅니다.
  설계 배경, 대안 검토, 트레이드오프는 PR 본문에 씁니다 — 코드 옆에 두면 구현이 바뀔 때마다 같이 낡습니다.
- **한 줄로 안 되면 주석을 늘리지 말고 코드를 고칩니다.** 이름을 바꾸거나 메서드를 추출하면
  설명이 필요 없어지는 경우가 대부분이고, 그래도 안 되면 그 복잡함 자체가 분리 신호입니다.
- **javadoc 형식(`/** */`, `@param`, `@return`, `{@link}`, `<p>`·`<b>` 태그)을 쓰지 않습니다.**
  외부에 배포하는 라이브러리가 아니라 IDE에서 코드를 직접 읽습니다. `//` 한 줄 주석으로 통일합니다.
